...
6. 发布成功后，立即把平台、链接、日期和备注补进 `docs/promotion/overseas-posting-log.md` 或对应跟踪文件，并纳入每日回访范围。

## 海外社媒执行频率

- 当前已纳入的每个海外社媒平台，每天目标发 1-2 条内容。
- 每个平台每天最多回复 5 条，不为了凑数回复低质量或不相关问题。
- 新平台只有在首次实际发布后，才补进 `docs/promotion/overseas-posting-log.md` 并纳入每日发帖、回复和回访范围。
