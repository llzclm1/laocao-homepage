...
- 主站 Factory Bridge 入口：中英文首页买家端和工厂端链接已指向 `factory.gewuji.dev`；首页已强制简化为 Hero、三入口、Contact、弱 Lab、Footer，并完成更明显的系统 serif 标题字体、亮度、色彩层级和正式 Contact 文案调整；部署后检查线上首页导航、Hero 背景图、三入口和移动端按钮是否生效。
