...
- 主站 Factory Bridge 入口：中英文首页买家端和工厂端链接已指向 `factory.gewuji.dev`；首页已强制简化为 Hero、三入口、Contact、弱 Lab、Footer，并完成更明显的系统 serif 标题字体、亮度、色彩层级和正式 Contact 文案调整；`/for-buyers/`、`/for-factories/`、`/field-materials/` 已统一为 GEWUJI / Factory Bridge 暖工业视觉系统。部署后检查线上首页导航、Hero 背景图、三入口、子页 Header / Footer、Field Materials 图片和移动端按钮是否生效。
- Factory 子域名源头：本仓库 `CNAME` 为 `gewuji.dev`，当前可控的是主站静态页面；`factory.gewuji.dev/` 根路径线上仍需确认对应部署源，避免误认为本仓库已经控制 factory 根入口页。
